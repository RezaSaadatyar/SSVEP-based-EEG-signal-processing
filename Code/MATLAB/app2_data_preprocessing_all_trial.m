    ...
    for j = 1:3    % Loop through each label 
        ...
        for k = 1:length(numb_trials) % Third loop ---> Sorting trials based on each label in the Signal
            ...
            if exist('data1', 'var') && (i > 1) && (j == 1)
                data1(:, :, size(data1, 3) + 1) = signal(time_start_trial(numb_trials(k)):time_start_trial(numb_trials(k)) + ...
                    duration_trial - 1, :);
            elseif exist('data2', 'var') && (i > 1) && (j == 2)
                data2(:, :, size(data2, 3) + 1) = signal(time_start_trial(numb_trials(k)):time_start_trial(numb_trials(k)) + ...
                    duration_trial - 1, :);
            elseif exist('data3', 'var') && (i > 1) && (j == 3)
                data3(:, :, size(data3, 3) + 1) = signal(time_start_trial(numb_trials(k)):time_start_trial(numb_trials(k)) + ...
                    duration_trial - 1, :);
            else
                data(:, :, k) = signal(time_start_trial(numb_trials(k)):time_start_trial(numb_trials(k)) + ...
                    duration_trial - 1, :);
            end
        end
