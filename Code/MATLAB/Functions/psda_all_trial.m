function psda = psda_all_trial(data, f_stim, num_sample_neigh, fs, num_harmonic)
...
psda = zeros(size(f_stim, 2), 1); % Initialize array to store PSDA values

for i = 1:size(f_stim, 2) % Loop over each stimulation frequency
    ps = zeros(size(data, 2), num_harmonic); % Initialize matrix to store PSD values for
    % each channel
    ...
    for j = 1:size(data, 2) % Loop over each channel
       ...
       x_fft = fft(xi, size(xi, 1)); % Perform Fourier transform
        ...
        for q = 1:num_harmonic % Loop over each harmonic
            ...
            ps(j, q) = 10 * log10((num_sample_neigh * max(psd(ind_fk))) / (sum(psd(ind_h)) ...
