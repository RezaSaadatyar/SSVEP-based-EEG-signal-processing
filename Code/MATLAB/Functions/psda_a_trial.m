function [max_freq, label]=psda_a_trial(fs, xi, num_sample_neigh, f_stim, num_harmonic, channel, trial)
...
% Start
% Inputs: Sampling frequency (fs), EEG signal data (xi), number of samples in neighborhood (num_sample_neigh),
...
for i = 1:size(f_stim, 2)
    ...
    for j = 1:num_harmonic
        ...
        % Calculate PSDA
        psda(i, j) = 10 * log10((num_sample_neigh * max(psd(indxfk))) / (sum(psd(ind)) - max(psd(indxfk))));
...
title(['PSDA; Trial: ' num2str(trial) '; Channel: ' num2str(trial)], 'FontSize', 10, 'FontWeight', 'bold')
