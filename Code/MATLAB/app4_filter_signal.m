...
filtered_data = filtering(x, f_low, f_high, order, fs, notch_freq, filter_active, ...
    notch_filter, quality_factor, type_filter);
trial = 1;
channel = 1;
...
fft_x_filter = fft(filtered_data(:, channel, trial)); % Perform Fourier transform on the filtered signal
...
title(['Freq stimulation=13Hz; Channel: ' num2str(channel) '; Trial: ' num2str(trial)], 'FontSize', 10, 'FontWeight', 'bold')
